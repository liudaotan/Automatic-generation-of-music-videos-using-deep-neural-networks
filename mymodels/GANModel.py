...
class DCGANGenerator(tr.nn.Module):
...
class SRGAN_Abstract_Generator(tr.nn.Module):
    def __init__(self):
        super().__init__()
...
class DCGAN_Abstract_Generator(tr.nn.Module):
    def __init__(self, ngpu):
        super(DCGAN_Abstract_Generator, self).__init__()
        self.ngpu = ngpu
        self.main = tr.nn.Sequential(
            # input is Z, going into a convolution
            tr.nn.ConvTranspose2d( ganconfig.nz, ganconfig.ngf * 8, 4, 1, 0, bias=False),
...
class Abstract_Generator(tr.nn.Module):
    ...
    def __init__(self, base_pth, boost_pth, ngpu=1):
        ...
        """
        super(Abstract_Generator, self).__init__()
        self.dc_generator = DCGAN_Abstract_Generator(ngpu=ngpu)
        self.sr_generaotr = SRGAN_Abstract_Generator()

        self.dc_generator.load_state_dict(tr.load(base_pth))
        self.sr_generaotr.load_state_dict(tr.load(boost_pth))

    def forward(self, x):
        x = self.dc_generator(x)
        x = self.sr_generaotr(x)
        return x